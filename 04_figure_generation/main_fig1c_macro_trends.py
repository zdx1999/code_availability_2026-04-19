...
def add_curve(ax, stat: pd.DataFrame, color: str, ylabel: str, title: str):
    if not stat.empty:
        ax.fill_between(stat['xmid'], stat['yq25'], stat['yq75'], color=COLOR_FILL, alpha=0.28)
        ax.plot(stat['xmid'], stat['ymed'], marker='o', lw=2.4, color=color)
    ax.set_ylabel(ylabel)
    ax.set_title(title, loc='left', fontweight='bold')


def annotate_sample(ax, n: int, text: str):
    ax.text(0.98, 0.05, f'{text}: n={n}', transform=ax.transAxes,
            ha='right', va='bottom', fontsize=9, color='0.35')


def main():
    ap = argparse.ArgumentParser(description='Build Fig.1c macro-trend panel set.')
    ap.add_argument('--full-csv', required=True, help='city_event_grid_full...csv')
    ap.add_argument('--out', required=True, help='Output PDF path')
    ap.add_argument('--figure-title', default='Figure 1c | Event-scale macro trends of flood hotspot reorganization')
    args = ap.parse_args()
    ...
    fig = plt.figure(figsize=(8.6, 7.2), dpi=300)
    gs = fig.add_gridspec(2, 1, height_ratios=[1.1, 0.95], hspace=0.25)
    ax1 = fig.add_subplot(gs[0, 0])
    ax2 = fig.add_subplot(gs[1, 0], sharex=ax1)

    add_curve(ax1, stat_total, COLOR_TOTAL,
              'Total event flood impacts',
              'a | Stronger rainfall increases overall event impacts')
    add_curve(ax2, stat_open, COLOR_SHARE,
              'Share of newly opened hotspots',
              'b | Newly opened hotspot share rises within extreme-event hotspot systems')

    ax2.set_xlabel('Event peak rainfall')
    ...
    fig.suptitle(args.figure_title, y=0.995, fontsize=13, fontweight='bold')
