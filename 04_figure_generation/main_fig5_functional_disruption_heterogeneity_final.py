...
ROAD_COLORS = {
    'Road L1': '#b8cbe0',   # deliberately lighter so L1 reads as secondary
    'Road L2': '#4f80a8',
    'Road L3': '#1f4e79',
}

# Everyday-use urban functions only for panel b
...
def road_summary(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Sort by median so L2/L3 read first in the panel
    if not out.empty:
...
def draw_interval(ax, summ: pd.DataFrame, xlabel: str, letter: str, color, lighten_labels: set[str] | None = None):
    ...
    for yi, (_, row), c in zip(y, summ.iterrows(), colors):
        lw = 1.9 if row['label'] in lighten_labels else 2.4
        alpha = 0.55 if row['label'] in lighten_labels else 0.95
        msize = 4.8 if row['label'] in lighten_labels else 5.8
...
def main():
    ap = argparse.ArgumentParser(description='Final refined Fig.5 without titles.')
    ...
    # Panel a: L2/L3 emphasized, L1 deliberately softened
    draw_interval(
        ax_a,
        road,
    ...
    # Panel b: focus on everyday functions only
    draw_interval(
        ax_b,
        poi,
    ...
    # Panel c: shorter y-axis label for alignment with other main figures
    draw_quartile_boxes(
        ax_c,
        panel_df,
